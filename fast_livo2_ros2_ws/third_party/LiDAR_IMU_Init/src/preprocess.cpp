...
//            added_pt.curvature = (pl_orig.points[i].timestamp - msg->header.stamp.toSec()) * 1000;  //s to ms
        ...
        if (plsize > 0) {
            first_timestamp = pl_orig.points[0].timestamp;
        }
        for (int i = 0; i < plsize; i++) {
            PointType added_pt;
            added_pt.normal_x = 0;
            ...
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            double offset_time = pl_orig.points[i].timestamp;
            if (offset_time > 1e12 && first_timestamp > 1e12) {
                offset_time -= first_timestamp;
            }
            added_pt.curvature = offset_time / 1e6;  // ns to ms
...
void Preprocess::livox_pcl2_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
    pcl::PointCloud<livox_pcl2::Point> pl_orig;
    ...
    for (int i = 0; i < plsize; i++) {
        ...
        added_pt.normal_y = 0;
        added_pt.normal_z = 0;
        added_pt.curvature = pl_orig.points[i].timestamp / 1e6;  // ns to ms

        double range = added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z;
        if (range < blind * blind || isnan(added_pt.x) || isnan(added_pt.y) || isnan(added_pt.z))
            continue;
