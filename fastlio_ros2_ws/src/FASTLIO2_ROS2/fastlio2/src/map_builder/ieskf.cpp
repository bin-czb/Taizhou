...
V21D State::operator-(const State &other) const
{
    ...
    delta.segment<3>(0) = Sophus::SO3d(projectToSO3(other.r_wi.transpose() * r_wi)).log();
    delta.segment<3>(3) = t_wi - other.t_wi;
    delta.segment<3>(6) = Sophus::SO3d(projectToSO3(other.r_il.transpose() * r_il)).log();
