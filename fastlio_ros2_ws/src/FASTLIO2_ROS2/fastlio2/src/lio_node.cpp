...
void fatalSignalHandler(int sig)
{
    ...
    std::cerr << "\n[FATAL] fastlio2 lio_node received signal " << sig << std::endl;
...
struct NodeConfig
{
    ...
    double imu_time_lag_to_lidar_s = 0.0;
    bool print_time_cost = false;
};
struct StateData
{
    bool lidar_pushed = false;
    bool warned_empty_cloud = false;
    bool warned_waiting_imu = false;
    std::mutex imu_mutex;
...
public:
    ...
    void loadParameters()
    {
        ...
        m_node_config.imu_acc_scale = config["imu_acc_scale"] ? config["imu_acc_scale"].as<double>() : 10.0;
        m_node_config.imu_time_lag_to_lidar_s =
            config["imu_time_lag_to_lidar_s"] ? config["imu_time_lag_to_lidar_s"].as<double>() : 0.0;
        m_node_config.print_time_cost = config["print_time_cost"].as<bool>();

        RCLCPP_INFO(this->get_logger(), "IMU topic: %s, LiDAR topic: %s, imu_acc_scale: %.6f, imu_time_lag_to_lidar_s: %.6f",
                    m_node_config.imu_topic.c_str(), m_node_config.lidar_topic.c_str(),
                    m_node_config.imu_acc_scale, m_node_config.imu_time_lag_to_lidar_s);
    ...
    void imuCB(const sensor_msgs::msg::Imu::SharedPtr msg)
    {
        ...
        double timestamp = Utils::getSec(msg->header) - m_node_config.imu_time_lag_to_lidar_s;
    ...
    void lidarCB(const livox_ros_driver2::msg::CustomMsg::SharedPtr msg)
    {
        ...
        if (cloud->empty())
        {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                 "LiDAR packet converted to an empty cloud. raw_points=%u, filter_num=%d, range=[%.2f, %.2f]",
                                 msg->point_num, m_builder_config.lidar_filter_num,
    ...
    bool syncPackage()
    {
        ...
        if (!m_state_data.lidar_pushed)
        {
            ...
            {
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "Drop empty LiDAR cloud before sync");
        ...
        if (m_state_data.last_imu_time < m_package.cloud_end_time)
        {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 3000,
                                 "Waiting for IMU to cover LiDAR scan. last_imu=%.6f, cloud_end=%.6f, dt=%.3f ms, imu_buffer=%zu, lidar_points=%zu",
                                 m_state_data.last_imu_time, m_package.cloud_end_time,
                                 (m_package.cloud_end_time - m_state_data.last_imu_time) * 1000.0,
                                 m_state_data.imu_buffer.size(), m_package.cloud->size());
            return false;
        }
        ...
        {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                 "Synced LiDAR scan has no IMU samples. Drop this scan. cloud_time=[%.6f, %.6f]",
                                 m_package.cloud_start_time, m_package.cloud_end_time);
            return false;
        }
        return true;
    }
    ...
    void timerCB()
    {
        ...
        m_state_data.synced_packages++;
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                             "Processing package #%zu: points=%zu, imus=%zu, cloud_time=[%.6f, %.6f]",
                             m_state_data.synced_packages, m_package.cloud->size(), m_package.imus.size(),
                             m_package.cloud_start_time, m_package.cloud_end_time);
        try
        {
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "MapBuilder exception: %s", e.what());
            return;
        }
        catch (const char *e)
        {
            RCLCPP_ERROR(this->get_logger(), "MapBuilder exception: %s", e);
            return;
        }
        catch (...)
        {
            RCLCPP_ERROR(this->get_logger(), "MapBuilder unknown exception");
            return;
        }
        ...
        {
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                                 "Builder not mapping yet. status=%d", static_cast<int>(m_builder->status()));
            return;
        }
